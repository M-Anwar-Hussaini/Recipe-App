...
RSpec.describe Recipe, type: :model do
  ...
  it "is  valid with a false 'public' value" do
    recipe = Recipe.new(name: 'Test Recipe', preparation_time: 10, cooking_time: 20, description: 'Test',
                        public: false, user:)
    expect(recipe).to be_valid
  end

  it 'has many recipe_foods and destroys them when deleted' do
    recipe = Recipe.create(name: 'Pizza', preparation_time: 15, cooking_time: 25, description: 'Homemade pizza',
                           public: true, user:)
    food1 = Food.create(name: 'Tomato', price: 1.5)
    food2 = Food.create(name: 'Cheese', price: 2.0)

    RecipeFood.create(recipe:, food: food1)
    RecipeFood.create(recipe:, food: food2)

    expect { recipe.destroy }.to change { RecipeFood.count }.by(0)
  end

  it 'belongs to a user' do
    recipe = Recipe.new(name: 'Salad', preparation_time: 10, cooking_time: 15, description: 'Healthy salad',
                        public: false, user:)
    expect(recipe.user).to eq(user)
  end

  it "toggles the 'public' attribute" do
    recipe = Recipe.create(name: 'Bread', preparation_time: 5, cooking_time: 20, description: 'Freshly baked bread',
                           public: false, user:)
  ...
  it 'calculates the total price of associated foods' do
    recipe = Recipe.create(name: 'Smoothie', preparation_time: 10, cooking_time: 5, description: 'Fruit smoothie',
                           public: true, user:)
    food1 = Food.create(name: 'Banana', price: 0)
    food2 = Food.create(name: 'Strawberry', price: 0, user:)
